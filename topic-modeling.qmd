...
First I'm going to build a set of LDA models with values of K (number of topics) ranging from 2 to 15. I'm also going to create a holdout dataset to calculate the heldout-likelihood metric later. This is somewhat akin to using a cross-validation holdout set in other machine learning methodologies.
...
jh_tune_models %>% 
  ...
  tidy(matrix = "gamma") %>%
  nest(data = -document) %>%
  mutate(data = map(data, ~ slice_max(.x, gamma, n = 1, with_ties = FALSE))) %>%
  unnest(data) %>%
  left_join(
    jh_clean %>%
      filter(
