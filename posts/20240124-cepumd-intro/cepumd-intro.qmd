...
::: columns
::: {.column width="37%"}
![](images/cepumd.png){style="padding: 50px 30px 0px 0px" fig-alt="cepumd hexsticker" fig-align="center" width="300"}
:::
...
You can install the development version of `cepumd` from the **arcenis-r/cepumd** repo on [GitHub](https://github.com/arcenis-r/cepumd) using your favorite installation method, e.g., `devtools::install_github()`, `remotes::install_github()`, etc.
...
Here I'm going to assume very little knowledge about the CE. I'd like to compare mean annual expenditures on food away from home between 2010 and 2020 by household size and I want to convert expenditures to 2023 dollars using the CPI. First I'd go to the [CE PUMD Data Files](https://www.bls.gov/cex/pumd_data.htm#csv) page and download the files that I need for both years. I'll also go to the [CE PUMD Documentation](https://www.bls.gov/cex/pumd_doc.htm) page to download the hierarchical grouping files to get the UCCs for "Food away from home" and the CE Dictionary to find out what variable has data on the household size.
...
Next I'll turn to finding the variable for household size in the CE data dictionary. It's important to remember that the dictionary is stored as an "XLSX" workbook that has three worksheets named `r sheet_names`. I'll use functions from the `readxl` package to work with the dictionary.
...
::: {.callout-note title="CPI series ID info"}
