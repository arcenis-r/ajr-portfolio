...
pkgs <- c("tidyverse", "timetk", "modeltime", "tidymodels")

invisible(
  lapply(pkgs, \(x) if (!x %in% .packages()) require(x, character.only = TRUE))
)
...
rm(list = ls()[!ls() %in% c("econ_data", "part4_mods", "hpi_wfset")])
