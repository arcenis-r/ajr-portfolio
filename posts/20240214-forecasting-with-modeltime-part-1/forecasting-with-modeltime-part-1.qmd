---
title: "Forecasting Case Shiller Index - Part I"
...
<a data-flickr-embed="true" href="https://www.flickr.com/photos/59937401@N07/5474453551" title="Price of Houses in the UK">![Price of Houses in the UK](https://live.staticflickr.com/5135/5474453551_afc6ded9af_w.jpg){alt="Price of Houses in the UK" width="400"}</a>
...
```{r}
#| label: import-case-shiller
#| cache: true

cs_data <- case_shiller_ids |>
  mutate(
    data = map(
      id,
      \(x) fredr(
  ...
  select(-id) |>
  unnest(data) |>
  select(city, date, case_shiller = value)
...
Here I ensured to clean up my environment a little and free up some of R's working memory.
...
econ_data <- list(cs_data, unemp_data, gas_data, cps_data, acs_pop_data) |>
...
-   `case_shiller`: Case-Shiller Home Price Index value
