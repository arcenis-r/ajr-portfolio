...
::: {.callout-note collapse="true" title="The 'assess' argument"}
...
arima_wflow_global <- workflow() |>   # <1>
  ...
  fit(training(econ_splits_global))   # <4>
...
arima_mtt_global <- modeltime_table(arima_wflow_global)
arima_mtt_global
```
...
arima_calib_global |>                            # <1>
  ...
  group_by(city) |>                              # <7>
  plot_modeltime_forecast(.interactive = FALSE)  # <8>
...
-   Get the time differences in running the initial processes

-   Get the accuracy tables at the city level for each process, merge, and compare

-   Show future forecast plots in a figure panel: <https://quarto.org/docs/authoring/figures.html#figure-panels>

## Additional model evaluation techniques

-   Breusch-Pagan Test

-   Ljung-Box test (maybe add to Part II)

-   Plot STL of residuals
