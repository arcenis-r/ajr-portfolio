---
title: "Dealing with Unbalanced Data: Optimal Threshold vs. Upsampling"
...
In any binary classification analysis it is common to have to deal with heavily unbalanced data, i.e., the frequency of the majority class in the dependent variable is overwhelmingly greater than that of the minority class. One of the main reasons this can be problematic is that many classificiation algorithms are biased toward the majority class. If 95% of the observations belong to the majority class in the training data and the algorithm always predicts the majority class, then it will have achieved 95% accuracy.

There are many ways of dealing with unbalanced data including changing from a classification algorithm to an anomaly detection algorithm like an isoforest or a one-class SVM, using SMOTE to rebalance the data sets in pre-processing, up- or down-sampling, and changing the classification threshold. Here I will only deal with up-sampling and changing the classification threshold to see how it affects classification metrics by way of going through an exercise. I'll first create a base model then generate an up-sampled model. I'll then find the optimal threshold for each. Finally I'll show a comparison of their respective classification metrics.
...
For the analysis I'll also be relying heavily on the following packages:

-   `tidyverse` [@tidyverse]: for data wrangling, visualization, and functional programming

-   `tidymodels`[@tidymodels]: for data pre-processing, model specification, model tuning, model evaluation, and extracting information from model objects

-   `probably`[@probably]: for selecting an optimal threshold

-   `themis`[@themis]: for extra pre-processing functions as for upsampling

-   `finetune`[@finetune]: for additional functions for model tuning

-   `colino`[@colino]: for the maximum Relevancy Minimum Redundancy (mRMR) supervised feature selection pre-processing step

-   `skimr`[@skimr]: for summarizing data

-   `knitr` [@knitr]: for table formatting

## Data Exploration

The first step is to load and summarize the data.
...
4.  Select variables based on mRMR; the percentil threshold used to decide which variables to keep will be chosen through model tuning
...
best_base <- select_best(base_tuned, "pr_auc")
...
# Get the workflow with the best MRMR threshold
best_upsample <- select_best(upsample_tuned, "pr_auc")
```
...
eval_upsample <- upsample_wflow |>
  finalize_workflow(select_best(upsample_tuned, "pr_auc")) |>
  last_fit(tt_split)
